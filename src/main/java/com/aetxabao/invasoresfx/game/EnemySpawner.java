...
public class EnemySpawner {
    ...
    public static List<AEnemy> createEnemies(Rect gameRect, int level) {
        ...
        switch (level){
            ...
            case 4:
            default:
                enemies = crearEnemigosNivelPrueba(gameRect);
        }
        return enemies;
    }
    ...
    public static EnemyShip createEnemyShip(EEnemyType type, Image enemyImage, Rect gameRect, int i, int j, int vx, int vy, EEnemyShot shot) {
        ...
        switch (type){
            ...
            case E_BARRIER:
            default:
            e = new EnemigoConBarrera(gameRect, enemyImage, TICKSxFRAME);
            break;
        }
        if (shot == E_SHOT_GUN){
            e.setWeapon(new Gun());
    ...
    public static List<AEnemy> crearEnemigosNivelPrueba(Rect gameRect) {
        List<AEnemy> enemies = new ArrayList<>();
        enemies.add(createEnemyShip(E_BARRIER, ENEMYSHIP_SPRITE_IMAGE_2, gameRect, 7, 0, -vx, vy, E_SHOT_GUN));
        enemies.add(createEnemyShip(E_BARRIER, ENEMYSHIP_SPRITE_IMAGE_2, gameRect, 7, 0, -vx, vy, E_SHOT_GUN));
        enemies.add(createEnemyShip(E_BARRIER, ENEMYSHIP_SPRITE_IMAGE_2, gameRect, 4, 0, vx, vy, E_SHOT_GUN));
        enemies.add(createEnemyShip(E_BARRIER, ENEMYSHIP_SPRITE_IMAGE_2, gameRect, 4, 0, vx, vy, E_SHOT_GUN));
        return enemies;
    }
